...
module.exports.run = async ({ api, event }) => {
    ...
    // Đoạn thơ thêm vào
    const tho = ["Lưu số em đi, khi nào má có cần con dâu thì gọi cho em.",
      
        "Anh yêu em rất nhiều, nhưng anh không muốn em có thể gặp anh.",
    ...
    const randomTho = tho[Math.floor(Math.random() * tho.length)];
      api.sendMessage(`⏱️ ${gio}\n🍩 Thính: ${randomTho}`, event.threadID, event.messageID);     
}