...
return (await require("axios").get(`https://api.dungkon.id.vn/catbox?url=${encodeURIComponent(uri)}`)).data.url;
}
async function api_link() {
  return (await require('axios').get('https://api.dungkon.id.vn/girl-video')).data.url;
}
