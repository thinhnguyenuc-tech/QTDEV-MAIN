 ...
 const fs = require("fs");
    ...
    module.exports.config = {
        ...
        hasPermssion: 0,
        credits: "qt",
        description: "Game bầu cua đặt cược",
        commandCategory: "Trò Chơi",
        usages: "<[𝗴𝗮̀/𝘁𝗼̂𝗺/𝗯𝗮̂̀𝘂/𝗰𝘂𝗮/𝗰𝗮́/𝗻𝗮𝗶] 𝗵𝗼𝗮̣̆𝗰[🐓/🦞/🍐/🦀/🐬/🦌]> <𝗦𝗼̂́ 𝘁𝗶𝗲̂̀𝗻 𝗰𝘂̛𝗼̛̣𝗰>",
        cooldowns: 5
    };

    module.exports.onLoad = async function () {
        if (!existsSync(__dirname + '/cache/ga.jpg')) {
            request('https://i.postimg.cc/T1HvhvPg/8v0kqu.jpg').pipe(createWriteStream(__dirname + '/cache/ga.jpg'));
        }
        if (!existsSync(__dirname + '/cache/tom.jpg')) {
            request('https://i.postimg.cc/7YfydhBV/jccxkv.jpg').pipe(createWriteStream(__dirname + '/cache/tom.jpg'));
        }
        if (!existsSync(__dirname + '/cache/bau.jpg')) {
            request('https://i.postimg.cc/sDqRrzqR/zolup2.jpg').pipe(createWriteStream(__dirname + '/cache/bau.jpg'));
        }
        if (!existsSync(__dirname + '/cache/cua.jpg')) {
            request('https://i.postimg.cc/VktPsTP0/0cnrfw.jpg').pipe(createWriteStream(__dirname + '/cache/cua.jpg'));
        }
        if (!existsSync(__dirname + '/cache/ca.jpg')) {
            request('https://i.postimg.cc/ZqFkpdRT/3gcwoh.jpg').pipe(createWriteStream(__dirname + '/cache/ca.jpg'));
        }
        if (!existsSync(__dirname + '/cache/nai.jpg')) {
            request('https://i.postimg.cc/63YD1xng/jstu0z.jpg').pipe(createWriteStream(__dirname + '/cache/nai.jpg'));
        }
        if (!existsSync(__dirname + '/cache/baucua.png')) {
            request('https://i.postimg.cc/05Tm3MnG/B-u-Cua-T-m-C-Truy-n-Th-ng.png').pipe(createWriteStream(__dirname + '/cache/baucua.png'));
        }
        if (!existsSync(__dirname + '/cache/baucua.gif')) {
            request('https://i.postimg.cc/7YCpXc6t/c25oj3.gif').pipe(createWriteStream(__dirname + '/cache/baucua.gif'));
        }
    };
        ...
        const slotItems = ["ga", "tom", "bau", "cua", "ca", "nai"];
            const moneyUser = (await Currencies.getData(event.senderID)).money;
                ...
                var moneyBet = parseInt(args[1]);
                    if (!args[0] || !isNaN(args[0])) return api.sendMessage({body: `🦀 === [ 𝗕𝗔̂̀𝗨 𝗖𝗨𝗔 ] === 🦀\n━━━━━━━━━━━━━━━━━━\n🦞𝗛𝘂̛𝗼̛́𝗻𝗴 𝗗𝗮̂̃𝗻 𝗦𝘂̛̉ 𝗗𝘂̣𝗻𝗴🦞\n\n ${global.config.PREFIX}𝗯𝗮𝘂𝗰𝘂𝗮 + 𝗰𝗮́ , 𝘁𝗼̂𝗺 , 𝗰𝘂𝗮 , 𝗴𝗮̀ , 𝗻𝗮𝗶 , 𝗯𝗮̂̀𝘂 + 𝘀𝗼̂́ 𝘁𝗶𝗲̂̀𝗻 𝗰𝘂̛𝗼̛̣𝗰 𝘁𝗿𝗲̂𝗻 (𝘃𝗱: ${global.config.PREFIX}𝗯𝗮𝘂𝗰𝘂𝗮 𝗴𝗮̀ 𝟮𝟬𝟬𝟬)\n\n⚠️𝗟𝘂̛𝘂 𝘆́ 𝗻𝗲̂́𝘂 𝗯𝗮̣𝗻 𝘀𝗽𝗮𝗺 𝗯𝗮𝘂𝗰𝘂𝗮 𝗵𝗲̣̂ 𝘁𝗵𝗼̂́𝗻𝗴 𝘀𝗲̃ 𝗯𝗮𝗻 𝗯𝗮̣𝗻!!!`, attachment: fs.createReadStream(__dirname + "/cache/baucua.png")} ,event.threadID, event.messageID);
            ...
            if (moneyBet < 1000) return api.sendMessage("[𝗗𝗮𝘄𝗻🐧] → 𝗦𝗼̂́ 𝘁𝗶𝗲̂̀𝗻 đ𝗮̣̆𝘁 𝗸𝗵𝗼̂𝗻𝗴 đ𝘂̛𝗼̛̣𝗰 𝗱𝘂̛𝗼̛́𝗶 𝟭𝟬𝟬𝟬 VND", event.threadID, event.messageID);
    ...
    var array = [number[0],number[1],number[2]];
        ...
        if (array.includes(itemm)) {
            ...
            await Currencies.decreaseMoney(event.senderID, parseInt(args[1])); console.log('s4')
                    return api.sendMessage({body:`==== 『 𝗬𝗢𝗨 𝗟𝗢𝗦𝗘 』 ====\n━━━━━━━━━━━━━━━━━━\n\n→ 𝗟𝗮̆́𝗰 𝗧𝗿𝘂́𝗻𝗴: ${full.join(" | ")}\n→ 𝗕𝗮̣𝗻 𝗧𝗵𝘂𝗮 𝗥𝗼̂̀𝗶 𝗩𝗮̀ 𝗕𝗶̣ 𝗧𝗿𝘂̛̀ ${args[1]} VND\n→ 𝗕𝗼𝘁 𝗛𝗼̂𝗻𝗴 𝗟𝗮̆́𝗰 𝗥𝗮 𝗖𝗼𝗻 ${icon}`,attachment: listimg},event.threadID, event.messageID);
