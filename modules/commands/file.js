module.exports.config = {
    ...
    version: '1.2.2',
    hasPermssion: 3,
    credits: 'Niio-team (DC-Nam) (qt fix 1 xí)',
    description: 'xem item trong folder, xóa, xem file, ghi chú',
...
module.exports.handleReply = function({ handleReply: $, api, event }) {
    try {
        ...
        if (isNaN(index) || !action) {
            return api.sendMessage("⚠️ Not found index file", event.threadID, event.messageID);
        }

        const d = $.data[index - 1];

        if (!['create', 'note'].includes(action)) if (!d && event.args[0]) return api.sendMessage('⚠️ Not found index file', event.threadID, event.messageID);
        ...
        switch (action) {
            ...
            case 'note': {
                ...
                axios.post('https://vitieubao.click/upload', { content })
                    .then(response => {
                        const { edit, raw } = response.data;
                        return send(`📝 Edit: ${edit}\n\n✏️ Raw: ${raw}\n────────────────\n📁 File: ${path}\n\n📌 Thả cảm xúc vào tin nhắn này để tải mã lên`).then(res => {
                            res = { ...res, name: 'fil', path, o: { api, event }, url: raw, action: 'confirm_upload' };
                            global.client.handleReaction.push(res);
                        });
                    })
                    .catch(e => send(`Lỗi: ${e.message}`));
                break;
            }
            default:
                api.sendMessage(`❎ Reply [open | send | del | view | create | zip | copy | rename | note] + stt`, event.threadID, event.messageID);
        };
    } catch (e) {
        console.error(e);
    ...
    try {
        ...
        switch (_.action) {
            case 'confirm_upload':
                ...
                writeFileSync(_.path, content);
                send(`✅ Nội dung đã được cập nhật thành công\n\n🔗 Tệp: ${_.path}`);
                break;
            default:
                break;
        }
...
function openFolder(a, b, c) {
    let folders_files = readdirSync(c).reduce((o, e) => {
        if (e === 'node_modules') return o;
        o[statSync(c + '/' + e).isFile() ? 1 : 0].push(e);
        return o;
    ...
    for (const i of [...folders_files[0], ...folders_files[1]]) {
        ...
        if (info.isDirectory()) {
            if (dest === _node_modules_path) continue;
    ...
    txt += `\n📊 Tổng dung lượng directory (không tính node_modules): ${convertBytes(bytes_dir)}\nReply [open | send | del | view | create | zip | copy | rename | note] + stt`;
...
function size_folder(folder = '') {
    ...
    for (let file of readdirSync(folder)) try {
        ...
        if (path === _node_modules_path) continue;
